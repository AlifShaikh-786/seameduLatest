...
const menuData = {
  aboutUs: [
    { label: "philosophy", href: "/department" },
    { label: "our team", href: "/course-page" },
    { label: "our partners", href: "/university-page" },
  ...
  programs: [
    {
      ...
      subCategories: [
        ...
        {
          title: "BBA",
          courses: [{ label: "Media and Communication", href: "#" }],
        },
      ],
    },
    {
    ...
    },
    {
      ...
      subCategories: [
        ...
        {
          title: "M.Des",
          courses: [{ label: "Interactive Design – UI/UX", href: "#" }],
        },
      ],
    },
    {
    ...
    <>
      ...
      >
        <div className="max-w-[1800px] mx-auto px-6 md:px-12 lg:px-20 flex items-center justify-between nav-container-custom">
          ...
          <Link
            href="/"
            className="relative z-[110] flex-shrink-0 group nav-logo-custom"
          >
              ...
              >
                <Link
                  ...
                  className={`relative px-5 py-2.5 ultra:px-7 ultra:py-3.5 text-[14px] ultra:text-[18px] font-bold tracking-tight transition-all duration-300 rounded-full flex items-center gap-1.5 nav-link-custom ${
                    isScrolled ? "text-slate-700" : "text-gray-400"
                  } hover:text-blue-600`}
                >
                  {item.label}
                  {item.key !== "placements" && (
                    <ChevronDown
                      size={14}
                      className={`opacity-50 transition-transform duration-300 nav-chevron-custom ${activeMenu === item.key ? "rotate-180" : ""}`}
                    />
                  )}
                  {/* Hover indicator */}
                  <motion.div
                    ...
                    animate={{
                      opacity: activeMenu === item.key ? 1 : 0,
                      scale: activeMenu === item.key ? 1 : 0.9,
                    }}
                  />
                </Link>
                        ...
                        {item.key === "programs" ? (
                          <>
                            ...
                            <div className="w-56 bg-slate-50/50 p-6 border-r border-slate-100 flex flex-col gap-2">
                              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-2 px-2">
                                Disciplines
                              </p>
                          ...
                          <div className="p-4 w-full flex flex-col gap-1">
                            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-3 px-3 mt-2">
                              Explore
                            </p>
                            {(menuData as any)[item.key]?.map((sub: any) => (
                              ...
                              >
                                ...
                                {sub.loc && (
                                  <span className="text-[10px] font-bold text-blue-600/60 uppercase tracking-widest mt-1">
                                    {sub.loc}
                                  </span>
                                )}
                              </Link>
          ...
          <div className="hidden nav:flex items-center gap-6">
            ...
            >
              <span className="relative z-10 flex items-center">Apply Now</span>
            </Link>
          </div>
                ...
                animate={
                  menuOpen
                    ...
                    : {
                        ...
                        y: 0,
                        width: "100%",
                        backgroundColor: isScrolled ? "#0f172a" : "#ffffff",
                      }
                }
              ...
              <>
                ...
                >
                  ...
                  <div className="flex-1 overflow-y-auto p-8 space-y-2">
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-6 px-2">
                      Navigation
                    </p>
                    {navItems.map((item) => (
                      <div key={item.key} className="group">
                        ...
                        >
                          ...
                          {item.key !== "placements" && (
                            <ChevronRight
                              size={18}
                              className={`transition-transform duration-300 ${expandedItem === item.key ? "rotate-90" : ""}`}
                            />
                          )}
                        </button>
                                  ...
                                  {item.key === "programs" && (
                                    <div className="space-y-4 pt-4">
                                      {menuData.programs.map((cat) => (
                                        ...
                                        >
                                          <h5 className="text-[10px] font-black uppercase tracking-widest text-blue-600/60">
                                            {cat.category}
                                          </h5>
                                            ...
                                            {cat.subCategories
                                              ...
                                              .map((course) => (
                                                <Link
                                                  key={course.label}
                                                  href={course.href}
                                                  className="text-sm font-bold text-slate-800 py-1"
                                                  onClick={() =>
                                                    setMenuOpen(false)
                                                  }
                                                >
