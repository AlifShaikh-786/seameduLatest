...
export default function NewsAndUpdatesSlider() {
  ...
  const handleScroll = (direction: "left" | "right") => {
    ...
    if (direction === "right") {
      ...
      if (target !== undefined)
        animate(x, target, { type: "tween", ease: "circOut", duration: 0.8 });
    } else {
  ...
  return (
    <section className="w-full bg-blue-50/50 py-10 md:py-10 overflow-hidden border-t border-blue-100/70 relative">
      <div className="max-w-[1500px] mx-auto px-6 sm:px-12 lg:px-32 transition-all duration-300">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-6">
          <div className="space-y-2">
            ...
            <p className="text-slate-500 text-md md:text-lg max-w-4xl font-medium">
              Stay updated with the latest news and events at Seamedu School of
              Pro Expression.
            </p>
          </div>
          ...
          >
            ...
            >
              {displayData.map((event, idx) => (
                <div
                  key={`${event.id}-${idx}`}
                  className="group flex flex-col bg-white rounded-xl overflow-hidden shadow-sm border border-slate-100/50 hover:border-blue-100 hover:shadow-md transition-all duration-500 shrink-0 w-[300px] sm:w-[300px] h-[420px] sm:h-[350px]"
                >
        ...
        <div className="flex justify-between sm:justify-end items-center px-0 mt-2 sm:-mt-4 relative z-10 translate-y-[-10px] sm:translate-y-0">
          <div className="flex items-center gap-2">
            <button
              ...
              className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl bg-white shadow-sm flex items-center justify-center border border-slate-200 active:scale-95 transition-all hover:bg-slate-50"
            >
